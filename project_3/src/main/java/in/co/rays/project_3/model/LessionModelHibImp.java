...
public class LessionModelHibImp implements LessionModelInt {
	...
	public void update(LessionDTO dto) throws ApplicationException, DuplicateRecordException {
		Session session = null;
		/*
		 * Transaction tx = null; LessionDTO exesistDto = findByLogin(dto.getLogin());
		 * 
		...
		try {
			...
			throw new ApplicationException("Exception in Lession update" + e.getMessage());
		} finally {
			session.close();
		}
	}
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		try {
			...
			throw new ApplicationException("Exception : Exception in  Lession list");
		} finally {
			session.close();
		}
		...
		try {
			...
			if (dto != null) {
				...
				if (dto.getName() != null && dto.getName().length() > 0) {
				  ...
				  if (dto.getChapter() != null && dto.getChapter().length() > 0) {
				  criteria.add(Restrictions.like("chapter", dto.getChapter() + "%"));
				  }
				/*
				 ...
				 */
				if (dto.getChapterName() != null && dto.getChapterName().length() > 0) {
					criteria.add(Restrictions.eq("chapterName", dto.getChapterName()));
				}
				/*
